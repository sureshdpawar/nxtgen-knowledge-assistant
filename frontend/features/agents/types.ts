...
export interface AgentCheckpointHistory {
  thread_id: string;
  checkpoints: AgentGraphState[];
}

export type AgentProgressEvent =
  | { type: "run_started"; run_id: string; thread_id?: string; agent_id: string; agent_name: string; tools: string[] }
  | { type: "llm_started"; iteration: number }
  | { type: "llm_completed"; iteration: number; duration_ms: number; has_tool_calls: boolean; tools: string[] }
  | { type: "tool_started"; name: string; args: Record<string, unknown> }
  | { type: "tool_completed"; name: string; duration_ms: number; output: unknown }
  | { type: "approval_required"; result: AgentRunResponse }
  | { type: "completed"; result: AgentRunResponse }
  | { type: "failed"; run_id?: string; thread_id?: string; message: string };
