...
export default function TestAgentDialog({
  ...
  const [
    result,
    setResult,
  ] = useState<AgentRunResponse | null>(
    null,
  );
      ...
      setMessages(
        (current) => {
          ...
          if (
            ...
            next.push({
              id:
                `assistant-resumed-${runResult.data.id}-${Date.now()}`,
              role: "assistant",
              content:
                resumedAnswer,
            });
  ...
  function handleProgressEvent(
    ...
    if (
      ...
      setRunId(
        approvalResult.run_id,
      );

      setThreadId(
        approvalResult.thread_id,
      );
      ...
      setMessages(
        (current) => [
          ...
          {
            id:
              `approval-${approvalResult.run_id}`,
            role: "system",
            content:
    ...
    if (
      ...
      setRunId(
        completedResult.run_id,
      );

      setThreadId(
        completedResult.thread_id,
      );
      ...
      if (
        ...
        setMessages(
          (current) => [
            ...
            {
              id:
                `assistant-${completedResult.run_id}`,
              role: "assistant",
              content:
                answer,
            },
  ...
  const graphState =
    graphStateQuery.data;

  const graphMessageCount =
    graphState
      ?.messages
      ?.length
    ?? 0;
              ...
              <div className="rounded-xl border bg-white p-5">
                ...
                {!runDetailsQuery.data && (
                  <p className="mt-3 text-sm text-slate-500">
                    Run details appear after execution starts.
                  </p>
                )}
