...
export default function OnlineEvaluationDashboard() {
  ...
  const summaryFilters = useMemo<OnlineEvalSummaryFilters>(
    () => ({
      knowledge_base_id: knowledgeBaseId || null,
      generator_provider: generatorProvider.trim() || null,
  ...
  } = useOnlineEvalSummary(summaryFilters);

  const processMutation = useProcessPendingOnlineEvals();

  async function handleProcessPending() {
    try {
      const response = await processMutation.mutateAsync({
        limit: processLimit,
        evaluator_llm_configuration_id: null,
      });

      toast.success(
        `Processed ${response.selected} online evaluation sample${
          response.selected === 1 ? "" : "s"
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className="sr-only">Process limit</label>
          <select
          ...
          >
            <RefreshCw
              className={`h-4 w-4 ${summaryFetching ? "animate-spin" : ""}`}
            />
            Refresh
          </button>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          ...
          <div>
            ...
            <input
              ...
              onChange={(event) => setGeneratorProvider(event.target.value)}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCard
          ...
          value={
            summaryLoading
              ? "…"
              : (summary?.outcomes.safe_abstentions ?? 0).toLocaleString()
          }
          subtitle="Insufficient retrieved evidence handled safely"
          icon={<ShieldCheck className="h-5 w-5" />}
        />
        ...
        <MetricCard
          ...
          value={
            summaryLoading
              ? "…"
              : (summary?.outcomes.failed ?? 0).toLocaleString()
          }
          subtitle="Completed evaluations with unsafe or poor outcomes"
        ...
        <MetricCard
          ...
          value={
            summaryLoading
              ? "…"
              : formatCost(
                  summary?.evaluation_cost.total,
                  summary?.evaluation_cost.currency,
                )
          }
          subtitle={
            summary
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard
          ...
          value={summaryLoading ? "…" : formatScore(summary?.average_scores.answer_relevancy)}
        ...
        <MetricCard
          ...
          subtitle={`${summary?.pending ?? 0} pending · ${summary?.running ?? 0} running · ${summary?.failed ?? 0} execution failed`}
          icon={<Activity className="h-5 w-5" />}
        />
      </div>

      <OnlineEvaluationResults
        baseFilters={summaryFilters}
