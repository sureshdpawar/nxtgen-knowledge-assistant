...
class WebsiteAgentService:
    ...
    async def start_session(
        ...
        *,
        ...
        # Invoke with a LangChain ToolCall rather than raw args.
        #
        # This matters for MCP tools using response_format="content_and_artifact":
        # LangChain then wraps the MCP result as a ToolMessage and preserves
        # structuredContent in message.artifact["structured_content"].
        tool_call = {
            "name": tool.name,
            "args": arguments,
            "id": f"website_session_{uuid4().hex}",
            "type": "tool_call",
        }
        ...
        logger.warning(
            ...
            "type=%s repr=%r",
            type(getattr(result, "artifact", None)),
        ...
        logger.warning(
            ...
            "type=%s repr=%r",
            type(getattr(result, "content", None)),
        ...
        logger.debug(
            "Website session-start tool normalized result "
            "channel=%s agent=%s tool=%s result_type=%s",
            channel.id,
            agent.id,
            tool_name,
            type(normalized_result).__name__,
        )
    ...
    def _normalize_tool_result(
        ...
        # MCP tools loaded by langchain-mcp-adapters commonly return a
        # ToolMessage when invoked with a ToolCall. Structured MCP output is
        ...
        if isinstance(value, dict):
            # Be tolerant of direct adapter-level structured result shapes.
            structured = (
                value.get("structured_content")
                or value.get("structuredContent")
            )
            ...
            # Standard LangChain text content block.
            if (
                value.get("type") == "text"
                and "text" in value
            ):
