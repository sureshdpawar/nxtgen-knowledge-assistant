...
from mcp_servers.academy.adapters.google_calendar import GoogleCalendarAdapter
from mcp_servers.academy.adapters.google_sheets import GoogleSheetsEnquiryAdapter
...
class AcademyEnquiryService:
    def __init__(self, settings: AcademyMCPSettings):
        ...
        self.sheets = GoogleSheetsEnquiryAdapter(settings)
        self.calendar = GoogleCalendarAdapter(settings)

    def create_enquiry(self, *, name: str, phone: str) -> dict:
    ...
    def update_enquiry(
        ...
        *,
        ...
        updated = self.sheets.update_enquiry(
            ...
            status="CALLBACK_REQUESTED" if callback_required else "OPEN",
        )
        if updated is None:
            return {
                "success": False,
    ...
    def schedule_consultation(
        ...
        *,
        ...
        enquiry = self.sheets.get_enquiry(enquiry_id.strip())
        if enquiry is None:
            return {
                "success": False,
                "enquiry_id": enquiry_id,
        ...
        meeting = self.calendar.schedule_consultation(
            enquiry_id=enquiry_id,
            visitor_name=enquiry.get("Name") or "Prospective Learner",
