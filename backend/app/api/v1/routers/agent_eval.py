...
def create_dataset(payload: AgentEvalDatasetCreate, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        return dataset_service.create(db=db, current_user=current_user, payload=payload)
...
async def import_dataset(file: UploadFile = File(...), db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    ...
    try:
        ...
        try:
            ...
            raise ValueError("Agent Evaluation dataset must be UTF-8 encoded.") from exc
        except json.JSONDecodeError as exc:
            raise ValueError(f"Invalid JSON: {exc.msg}.") from exc
        if not isinstance(data, dict):
            raise ValueError("Agent Evaluation dataset JSON must contain one object.")
        try:
            payload = AgentEvalDatasetImportPayload.model_validate(data)
        except ValidationError as exc:
            raise ValueError(f"Invalid Agent Evaluation dataset: {exc}") from exc
        dataset, case_count = dataset_import_service.import_dataset(
...
def list_datasets(agent_id: UUID | None = None, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        return dataset_service.list(db=db, current_user=current_user, agent_id=agent_id)
    except ValueError as exc:
        raise bad_request(exc) from exc
...
def get_dataset(dataset_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        dataset = dataset_service.get(db=db, current_user=current_user, dataset_id=dataset_id)
    except ValueError as exc:
        raise bad_request(exc) from exc
    if dataset is None:
        raise HTTPException(status_code=404, detail="Agent Evaluation dataset not found.")
    return dataset
...
def create_case(dataset_id: UUID, payload: AgentEvalCaseCreate, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        return case_service.create(db=db, current_user=current_user, dataset_id=dataset_id, payload=payload)
    except ValueError as exc:
        raise bad_request(exc) from exc
...
def list_cases(dataset_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        return case_service.list(db=db, current_user=current_user, dataset_id=dataset_id)
    except ValueError as exc:
        raise bad_request(exc) from exc
...
def delete_case(dataset_id: UUID, case_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        case_service.delete(db=db, current_user=current_user, dataset_id=dataset_id, case_id=case_id)
    except ValueError as exc:
        raise bad_request(exc) from exc
...
def create_experiment(payload: AgentEvalExperimentCreate, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        return experiment_service.create(db=db, current_user=current_user, payload=payload)
    except ValueError as exc:
        raise bad_request(exc) from exc
...
def list_experiments(dataset_id: UUID | None = None, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        return experiment_service.list(db=db, current_user=current_user, dataset_id=dataset_id)
    except ValueError as exc:
        raise bad_request(exc) from exc
...
def get_experiment(experiment_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        experiment = experiment_service.get(db=db, current_user=current_user, experiment_id=experiment_id)
    except ValueError as exc:
        raise bad_request(exc) from exc
    if experiment is None:
        raise HTTPException(status_code=404, detail="Agent Evaluation experiment not found.")
    return experiment
...
async def run_experiment(experiment_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        return await experiment_service.run(db=db, current_user=current_user, experiment_id=experiment_id)
    except ValueError as exc:
        raise bad_request(exc) from exc
...
def list_experiment_results(experiment_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    try:
        return experiment_service.list_results(db=db, current_user=current_user, experiment_id=experiment_id)
    except ValueError as exc:
        raise bad_request(exc) from exc
