...
from app.services.evaluators.retrieval_set_metrics import (
...
class EvaluatorRegistry:
    ...
    def __init__(self):
        ...
        # Retrieval metrics.
        self.register(HitAtKEvaluator())
        self.register(PrecisionAtKEvaluator())
        self.register(RecallAtKEvaluator())
        self.register(ReciprocalRankEvaluator())

        # Generation metrics.
        self.register(FaithfulnessEvaluator())
        self.register(AnswerRelevancyEvaluator())
        self.register(CorrectnessEvaluator())
        self.register(RefusalCorrectnessEvaluator())

    def register(
        self,
        evaluator: BaseEvaluator,
    ) -> None:
        metric_name = evaluator.metric_name.strip().lower()

        if not metric_name:
            raise ValueError(
                "Evaluator metric_name cannot be empty."
            )
