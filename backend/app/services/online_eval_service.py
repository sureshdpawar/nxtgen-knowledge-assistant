...
class OnlineEvalService:
    """
    Execute asynchronous production quality
    evaluation for sampled RAG interactions.

    V1 uses three reference-free metrics:

    - faithfulness
    - answer relevancy
    - contextual relevancy

    source_trace_id remains the trace ID from
    the original user-facing production request.
    The asynchronous judge execution receives
    its own OpenTelemetry trace.
    """
...
A high score means:
- the retrieved context directly helps answer
  the question
- most supplied context is useful for the
  question
- irrelevant or distracting context is limited

A low score means:
- most retrieved context is unrelated
- the context does not provide useful evidence
  for answering the question
- the retrieved material is substantially noisy

Judge the Retrieved Context against the Question.
Do NOT evaluate the factual quality of the
Actual Answer in this metric.
Do NOT use outside knowledge.
""".strip()
    ...
    def _total_cost(
        ...
        Unknown pricing remains explicit and
    ...
    def process_pending(
        ...
        *,
        ...
        This is intentionally synchronous at the
        service boundary. It can later be called
        by an API endpoint, CLI command, scheduled
        process, Celery worker, or another queue
        consumer without changing evaluation logic.
        """
