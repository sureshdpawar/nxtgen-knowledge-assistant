...
from app.agents.a2a_tool_provider import (
    A2AAgentToolProvider,
)
...
class AgentToolRegistry:
    ...
    def __init__(self):
        self.rest_tool_factory = RESTToolFactory()
        self.mcp_tool_provider = MCPToolProvider()
        self.a2a_tool_provider = A2AAgentToolProvider()
    ...
    async def get_tools(
        ...
        if knowledge_base_ids:
            knowledge_tool = create_knowledge_search_tool(
                db=db,
        ...
        definitions = self._get_assigned_tools(
            db=db,
            tenant_id=tenant_id,
            tool_ids=tool_ids,
        )
        ...
        for definition in definitions:
            if definition.tool_type != ToolType.REST:
                continue
            ...
            rest_tool = self.rest_tool_factory.create(
                tool=definition,
        ...
        for (
            integration_id,
            mcp_definitions,
        ) in mcp_definitions_by_integration.items():
            integration = mcp_definitions[0].integration
            if integration is None:
                continue

            try:
                selected_tools = await self.mcp_tool_provider.get_assigned_tools(
                    integration=integration,
                ...
                logger.exception(
                    "MCP tool discovery failed integration=%s",
                    integration_id,
                )
                raise

        if agent_id is not None:
            a2a_tools = await self.a2a_tool_provider.get_connected_agent_tools(
                db=db,
                tenant_id=tenant_id,
        ...
        logger.info(
            "Agent tools resolved knowledge_bases=%s "
            "assigned_definitions=%s runtime_tools=%s",
