...
class OnlineEvalProcessResponse(BaseModel):
    selected: int
    completed: int
    failed: int


class OnlineEvalResultRead(BaseModel):
...
class OnlineEvalResultSummary(BaseModel):
    model_config = ConfigDict(from_attributes=True)
...
class OnlineEvalAverageScores(BaseModel):
    faithfulness: float | None
    answer_relevancy: float | None
    contextual_relevancy: float | None


class OnlineEvalCostSummary(BaseModel):
    total: float | None
    currency: str | None
...
class OnlineEvalOutcomeSummary(BaseModel):
    answered_successfully: int
    safe_abstentions: int
    failed: int
    unclassified: int


class OnlineEvalSummaryRead(BaseModel):
    total: int
    pending: int
    running: int
    completed: int
